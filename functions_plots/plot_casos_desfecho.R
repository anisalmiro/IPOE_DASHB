
#raw_base_rename$hospitalizacao <- toupper(raw_base_rename$hospitalizacao)

casos_desfecho <- raw_base_rename %>%
  dplyr::filter(!is.na(desfecho)) %>%  # Remover linhas com faixa_etaria NA 
  group_by(desfecho) %>%
  dplyr::summarise(Casos = n()) 


 # Criar o gráfico de colunas empilhadas
 Plot_casos_desfecho <- hchart(
   casos_desfecho, 
   'bar', 
   hcaes(x = desfecho, y = Casos, group = desfecho),
   dataLabels = list(enabled = TRUE, style = list(fontSize = "11px")),
   stacking = "normal"
 ) %>%
   hc_colors(c("#32A594","#EFC000FF", "#ad2f30","#a0a0ed","#203d7e", '#00a1cd')) %>%
   hc_yAxis(labels=list(
     style= list (
       fontSize= '16px',
       color= 'black'
     ))) %>% 
   hc_xAxis(labels=list(
     style= list (
       fontSize= '12px',
       color= 'black'
     ))) %>% 
   hc_tooltip(crosshairs = TRUE,
              shared = TRUE, borderWidth = 5) %>%
   hc_title(
     text = "<b><i></i>Regime de Atendimento</b>",
     margin = 20,
     align = "left",
     style = list(color = "#22A884", useHTML = TRUE)
   ) %>%
   hc_add_theme(hc_theme_google()) %>%
   hc_exporting(enabled = TRUE) 
 
 